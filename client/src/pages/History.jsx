...
export default function History() {
  ...
  return (
    <div>
      <h1 className="page-title mb-6 font-semibold text-slate-900">Session History</h1>

      <Card className="mb-6">
        <div className="grid grid-cols-1 gap-3 xs:grid-cols-2 md:grid-cols-4">
          <input
            placeholder="Plate number"
            className="min-h-11 rounded-md border border-slate-300 px-3 text-base md:text-sm"
        ...
        <button
          onClick={() => load(1)}
          className="mt-3 min-h-11 w-full rounded-md bg-slate-900 px-4 text-sm font-medium text-white hover:bg-slate-800 xs:w-auto"
        >
          Search
        </button>
      </Card>
      ...
      {result.sessions.length === 0 ? (
        <Card>
          <p className="py-2 text-center text-sm text-slate-500">No sessions found.</p>
        </Card>
      ) : (
        <>
              ...
              <div key={s._id} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                <div className="mb-2 flex items-start justify-between gap-3">
                  ...
                  <div className="text-right">
                    <div className="text-base font-semibold text-slate-900">{formatMoney(s.chargeAmount)}</div>
                    <div className="text-xs uppercase text-slate-500">{s.paymentMethod}</div>
                  </div>
                </div>
                ...
                <div className="mt-1 text-xs text-slate-500">Duration: {formatDuration(s.durationMinutes)}</div>
              </div>
            ))}
          </div>
                ...
                <tbody>
                  {result.sessions.map((s) => (
                    <tr key={s._id} className="border-b border-slate-100">
                      ...
                      <td className="py-3 pr-4 text-slate-600">{formatIST(s.exitTime)}</td>
                      <td className="py-3 pr-4 text-slate-600">{formatDuration(s.durationMinutes)}</td>
                      <td className="py-3 pr-4 font-medium text-slate-900">{formatMoney(s.chargeAmount)}</td>
                      <td className="py-3 uppercase text-slate-600">{s.paymentMethod}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      <div className="mt-4 flex flex-col items-center justify-between gap-3 xs:flex-row">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="min-h-11 rounded-md border border-slate-300 px-4 text-sm disabled:opacity-50"
          >
            Previous
          </button>
          <button
