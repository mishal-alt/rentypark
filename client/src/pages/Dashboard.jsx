...
import { Car, IndianRupee, LogIn, Grid3x3 } from 'lucide-react';
  ...
  return (
    <div>
      <div className="mb-6 md:mb-8">
        <h1 className="page-title font-semibold text-slate-900">Dashboard</h1>
        <p className="mt-1 text-sm text-slate-500">Live overview of your station, right now.</p>
      </div>
        ...
        <Card title="Occupancy by Vehicle Type">
          {occupancy.length === 0 ? (
            <p className="text-sm text-slate-500">
              ...
              <Link to="/slots" className="text-blue-600 hover:underline">
                Set up slots
              </Link>
            </p>
          ) : (
                ...
                return (
                  <li key={o.type}>
                    <div className="mb-1.5 flex items-center justify-between text-sm">
                      <span className="font-medium capitalize text-slate-700">{o.type}</span>
            ...
            <ul className="space-y-2">
              {active.slice(0, 6).map((s) => (
                <li key={s._id} className="flex items-center justify-between text-sm">
                  <span className="font-medium text-slate-800">{s.plateNumber}</span>
                  <span className="text-slate-500">{formatIST(s.entryTime)}</span>
                </li>
              ))}
            </ul>
          )}
          <Link to="/active" className="mt-4 inline-block text-sm font-medium text-blue-600 hover:underline">
